...
8. **`eas update` does NOT inherit eas.json's `env` block.** `eas build --profile production` bakes env vars from `eas.json` into the native bundle, but `eas update --branch production` builds the JS bundle from whatever's in your local shell. If `EXPO_PUBLIC_*` aren't in shell, they bundle as `undefined`, Supabase init fails, and `ConfigurationErrorScreen` ships to users. The canonical fix is to source `.env.local` into the shell before running update:
   ```bash
   cd apps/mobile && set -a && source .env.local && set +a && eas update --branch production --message "..."
   ```
   **Do NOT pass `--environment production`** — that flag pulls from EAS *server-side* environment variables (set via `eas env:create` or the EAS dashboard), which are unconfigured for this project. Passing it overrides `.env.local` auto-loading and ships an empty bundle. The old `--build-profile production` flag (that DID read eas.json's env block) was removed in eas-cli 17.x. Hit this on 2026-06-09: `--environment production` shipped a broken OTA to production users.
9. **Metro caches Babel transforms — including broken ones.** Related to #8. If an `eas update` runs with env vars missing/empty, Metro caches the uninlined transform of [env.ts](apps/mobile/src/lib/env.ts) (where `process.env.EXPO_PUBLIC_*` reads stay literal instead of getting replaced with string values). Every subsequent `eas update` reuses that cached transform and produces a byte-identical broken bundle — same `entry-<hash>.hbc` filename, same ConfigurationErrorScreen on device, even after env vars are correctly in shell. The cache lives at `/private/var/folders/*/T/metro-cache`. Purge it before retrying:
   ```bash
   rm -rf /private/var/folders/*/T/metro-cache apps/mobile/.expo apps/mobile/dist
   ```
   Spot-check a fresh bundle has the values: `grep -aoc "<unique-fragment-of-an-env-value>" apps/mobile/dist/_expo/static/js/ios/entry-*.hbc` — non-zero = inlined. Bundle hash changing across runs is also a good signal (cache miss + Metro re-ran the transform). Hit this on 2026-06-10 launch day; took an hour and a rollback to embedded to recover.
