...
export default function UserProfileScreen() {
  ...
  const { data: stats } = useUserSocialStats(id);
  const toggleFollow = useToggleFollow();
  const isSelf = !!profile && !!selfId && profile.id === selfId;

  // Self gets the full list (incl. private). Others get the public-only view.
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <View style={{ alignItems: 'center', gap: 12, paddingVertical: 24 }}>
              ...
              {/* Primary action: Edit for self, Follow/Following for others */}
              {isSelf ? (
